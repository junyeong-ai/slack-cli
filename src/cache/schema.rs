...
pub fn initialize_schema_sync(pool: &Pool<SqliteConnectionManager>) -> CacheResult<()> {
    ...
    conn.execute_batch(
        ...
        CREATE VIRTUAL TABLE IF NOT EXISTS users_fts USING fts5(
            id UNINDEXED, name, display_name, real_name, email,
            content=users, content_rowid=rowid, tokenize='porter unicode61'
        );

        CREATE VIRTUAL TABLE IF NOT EXISTS channels_fts USING fts5(
            id UNINDEXED, name, topic, purpose,
            content=channels, content_rowid=rowid, tokenize='porter unicode61'
        );
